...
.DESCRIPTION
    ...
    Also writes a reconciliation report (tools/PfbValueEnumReconciliation.md) comparing
...
.PARAMETER OutputPath
    Where to write the manifest. Defaults to Data/PfbValueEnumMap.json relative to the
    repo root (one level up from tools/).
.PARAMETER ReconciliationPath
    Where to write the reconciliation report. Defaults to
    tools/PfbValueEnumReconciliation.md relative to this script.
.EXAMPLE
...
if (-not $OutputPath) {
    ...
    $OutputPath = Join-Path (Join-Path $repoRoot 'Data') 'PfbValueEnumMap.json'
}
if (-not $ReconciliationPath) {
    $ReconciliationPath = Join-Path $scriptDir 'PfbValueEnumReconciliation.md'
}
...
$mdLines.Add('')
$mdLines.Add("Generated by ``tools/Build-PfbValueEnumMap.ps1`` against ``Data/PfbValueEnumMap.json`` ($($processedVersions.Count) REST versions, $($entries.Count) entries).")
$mdLines.Add('')
